...
test('nega gravação de feedback através de symlink interno', async () => {
  const root = await fixture(); await mkdir(path.join(root, '.draftroom')); await writeFile(path.join(root, 'untouched.txt'), 'keep'); await symlink(path.join(root, 'untouched.txt'), path.join(root, '.draftroom/feedback.jsonl')); const store = new WorkspaceStore(root);
...
describe('Ajustes visuais e fluxos',()=>{
 test('preserva ajustes sucessivos sem alterar o HTML e restaura o original',async()=>{
  const root=await fixture();try{
   ...
   const reopened=await new WorkspaceStore(root).snapshot();
   assert.deepEqual(reopened.edits?.[0].styles,{color:'#123456',padding:'12px'});assert.equal(reopened.edits?.[0].text,'Novo texto');
   assert.equal(await readFile(path.join(root,'frames/one/index.html'),'utf8'),before);
   await store.saveEdit({frameId:'one',selector:'button',remove:true});assert.deepEqual((await store.snapshot()).edits,[]);
  }finally{await rm(root,{recursive:true});}
 });

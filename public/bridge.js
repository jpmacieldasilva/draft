(() => {
  let mode = 'interact', outline, dragging, selected, originals = new Map();
  ...
  const fontLonghands = new Set(['fontSize','lineHeight','fontWeight','fontFamily','fontStyle','fontVariant']);
  ...
  function prepareTypography(element, styles) {
    if(Object.keys(styles).some(name=>fontLonghands.has(name))) element.style.font='';
  }
  function preview(change) {
    if(!change||typeof change!=='object'||typeof change.selector!=='string'||!change.styles||typeof change.styles!=='object') return;
    const element=find(change.selector);
    if(!element) return;
    if(!validStyles(change.styles)) { report(element,'Use cores em hexadecimal ou RGB e valores numéricos válidos.'); return; }
    ...
    if(!originals.has(element)) originals.set(element,{style:element.getAttribute('style'),text:element.childElementCount===0?element.textContent:undefined});
    prepareTypography(element,change.styles);
  ...
  addEventListener('message',event=>{
    ...
    if(message.type==='draftroom:mode'&&['interact','element','comment'].includes(message.mode)) {mode=message.mode;dragging=undefined;clear();}
